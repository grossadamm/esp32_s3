...
class FinanceHTTPServer {
    ...
    async getMonthlySpending(months) {
        const daysBack = months * 30;
        const cutoffDate = new Date();
        cutoffDate.setDate(cutoffDate.getDate() - daysBack);
        const rows = await this.db.all(`SELECT category, SUM(ABS(amount)) as total
             FROM transactions 
             WHERE date >= ? AND amount < 0
             GROUP BY category
             ORDER BY total DESC`, [cutoffDate.toISOString().split('T')[0]]);
        const spending = {};
        for (const row of rows) {
    ...
    async getRecentTransactions(days, limit) {
        const cutoffDate = new Date();
        cutoffDate.setDate(cutoffDate.getDate() - days);
        const rows = await this.db.all(`SELECT date, description, amount, category
             FROM transactions 
             WHERE date >= ?
             ORDER BY date DESC
             LIMIT ?`, [cutoffDate.toISOString().split('T')[0], limit]);
        return rows.map(row => ({
            date: row.date,
    ...
    async getLargeExpenses(threshold, days) {
        const cutoffDate = new Date();
        cutoffDate.setDate(cutoffDate.getDate() - days);
        const rows = await this.db.all(`SELECT date, description, amount, category
             FROM transactions 
             WHERE date >= ? AND amount < ? AND amount < 0
             ORDER BY amount ASC`, [cutoffDate.toISOString().split('T')[0], -threshold]);
        return rows.map(row => ({
            date: row.date,
            description: row.description,
