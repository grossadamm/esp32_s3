...
class OptionsImporter {
    ...
    calculateExpirationDate(grantDate) {
        if (!grantDate)
            return '';
    ...
    async importOptions(filename) {
        ...
        for (const row of grantRecords) {
            try {
                ...
                const existing = await this.db.get('SELECT id FROM stock_options WHERE grant_number = ?', [grantNumber]);
                if (existing) {
                    skippedCount++;
                    continue;
                }
...
//# sourceMappingURL=options-import.js.map