...
class TaxAwareRetirementCalculator {
  ...
  private readonly EXCLUDED_SPENDING = new Set([
  ...
  private async getOptionsAnalysis(): Promise<TaxAnalysis> {
    return new Promise((resolve, reject) => {
      this.db.get(`
        SELECT 
  ...
  private async getAnnualSpending(): Promise<number> {
    return new Promise((resolve, reject) => {
      this.db.all(`
        SELECT 
          category,
  ...
  private async getAnnualIncome(): Promise<number> {
    return new Promise((resolve, reject) => {
      this.db.get(`
      ...
      `, [], (err, row: any) => {
        if (err) reject(err);
        else resolve(row?.total_income || 0);
      });
    });
  }

  private calculateYearsToTarget(currentAmount: number, targetAmount: number, annualSavings: number, growthRate: number = 0.07): number {
    if (annualSavings <= 0 || currentAmount >= targetAmount) return 0;
  ...
  private calculateScenarios(spending: number, netWorth: number, annualSavings: number): RetirementScenario[] {
  ...
  private generateRecommendations(analysis: RetirementAnalysis): string[] {
  ...
  private async getUrgentActions(): Promise<string[]> {
    return new Promise((resolve, reject) => {
      this.db.get(`
        SELECT 
  ...
  async calculateRetirement(): Promise<RetirementAnalysis> {
