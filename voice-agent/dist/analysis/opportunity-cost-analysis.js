...
class OpportunityCostAnalyzer {
    ...
    async fetchSP500Data() {
      ...
      `);
            ...
            for (const [date, values] of Object.entries(timeSeries)) {
                ...
                try {
                    await this.db.run('INSERT OR REPLACE INTO sp500_prices (date, close) VALUES (?, ?)', [date, parseFloat(dayData['4. close'])]);
                    imported++;
                }
                catch (error) {
                    // Skip errors
    ...
    async getOptionsInvestmentByYear() {
      ...
      ORDER BY grant_date
    `);
        const investmentByYear = new Map();
    ...
    async calculateSP500Returns() {
        ...
        const startPrice = await this.db.get('SELECT close FROM sp500_prices WHERE date >= ? ORDER BY date ASC LIMIT 1', [startDate]);
        const endPrice = await this.db.get('SELECT close FROM sp500_prices WHERE date <= ? ORDER BY date DESC LIMIT 1', [endDate]);
    ...
    async performOpportunityCostAnalysis() {
...
// Run analysis
main();
//# sourceMappingURL=opportunity-cost-analysis.js.map