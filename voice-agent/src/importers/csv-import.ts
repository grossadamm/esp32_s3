...
class CSVImporter {
  ...
  private generateTransactionId(csvRow: CSVTransaction, index: number): string {
  ...
  private async ensureAccountExists(accountName: string): Promise<string> {
  ...
  async importCSV(filename: string): Promise<void> {
    console.log('📂 Reading CSV file...');
    
    const csvContent = fs.readFileSync(filename, 'utf-8');
    const records: CSVTransaction[] = parse(csvContent, {
      columns: true,
